  ...
  return (
    <div
      className={
        ...
        location.pathname === "/inspection" || location.pathname === "/*"
          ? "bg-success w-100 "
          : "bg-transparent UserNavTransparent z-2 position-absolute top-0 w-100"
      }
    >
      <div className="switch position-absolute border rounded-pill d-lg-flex gap-3 p-2 d-none " onClick={setLightMode}>
      ...
      <header className="usernavheader align-items-center px-lg-5 px-2  py-4 d-flex">
        ...
        <nav className="d-lg-flex  d-none flex-lg-row flex-column align-items-center justify-content-evenly  position-md-absolute top-100 end-0">
          ...
          <ul className="UserNavLinks d-flex pt-3 align-items-center gap-5 ">
            <li>
              <Link to="/" className={location.pathname === "/" ? "text-success fw-bold " : "text-white fw-bold "}>
                Home
              </Link>
            </li>
            <li>
              {" "}
              <Link to="/properties" className= {location.pathname === "/properties" ? "text-success fw-bold " : "text-white fw-bold "}>
                Properties
              </Link>
            </li>
            <li>
              {" "}
              <Link to="/contact" className= {location.pathname === "/contact" ? "text-success fw-bold " : "text-white fw-bold "}>
                Contact Us
              </Link>
            </li>
            <li>
          ...
          <div className="d-flex flex-column ms-xxl-5  flex-lg-row  gap-3 align-items-center mt-sm-3 ">
            <Link to="/signup">
              <button className={location.pathname === `/properties/${propertyId}` || location.pathname === "/inspection" ? " btn border bg-success text-white px-4 py-2" : "btn border px-4 py-2 text-success bg-white"}> 
                Sign Up
              </button>
            </Link>
            <Link to="/login">
              <button className={location.pathname === `/properties/${propertyId}` || location.pathname === "/inspection" ? "btn bg-white border-0 px-5 py-2 text-success " : "btn text-white px-4 py-2 bg-success"}>
                Log In
              </button>
            </Link>
          </div>
